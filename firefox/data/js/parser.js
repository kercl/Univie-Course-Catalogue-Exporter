	...
	"compile": 
		function() {
			...
			for(var i = 0; i < this.patternStrings.length; i++) {
				...
				var cutted = string_cut(this.patternStrings[i], this.targets);
				...
				for(var j = 0; j < cutted.length; j++) {
					if(this.targets.indexOf(cutted[j]) >= 0) {
						...
						brackets = brackets + string_count(cutted[j], "(");
						...
						checkbox.onclick = function(curcb, course) {
									return function() {
										if(curcb.checked) {
											...
											self.port.emit("parser-result", {instruction: "enable", courses: pc, debug: "TEST_ENABLE"});
											//chrome.runtime.sendMessage({instruction: "enable", courses: pc, debug: "TEST_ENABLE"});
										}else {
											pc = parseEntry(course);
											self.port.emit("parser-result", {instruction: "disable", courses: pc, debug: "TEST_ENABLE"});
